    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-purple-600 via-purple-500 to-purple-400 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4"><a href="/other-repair" className="inline-flex items-center gap-1.5 text-white/60 hover:text-white text-sm font-medium transition-colors"><ArrowLeft size={14} /> {t('其他设备', 'Other Devices')}</a></div>
          ...
          <p className="text-green-100 max-w-2xl">{t('Camera Find、Reno、A系列——屏幕碎了、电池不耐用了、充电口坏了，拿来给我看看。2007年至今奋斗在维修一线。', 'Camera Find, Reno, A series — cracked screen, battery drain, charging port issues. On the job since 2007.')}</p>
          <div className="flex gap-3 mt-6">
            <button onClick={() => setShowContact(true)} className="bg-white text-purple-600 font-semibold px-5 py-2.5 rounded-xl hover:bg-purple-50 shadow-lg">{t('📱 微信咨询', '📱 WeChat')}</button>
            <a href="https://wa.me/6596146709?text=Camera手机需要维修" target="_blank" className="bg-green-500 text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-green-600 shadow-lg">{t('💬 WhatsApp', '💬 WhatsApp')}</a>
          </div>
        </div>
      </section>
      <section className="py-14">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[{title:'相机维修',titleEn:'Screen',desc:'Camera Find/Reno/A系列相机维修，OLED/LCD。碎裂漏液触摸不灵。'},
              {title:'电池更换',titleEn:'Battery',desc:'Camera电池不耐用、鼓包、充不进电。原装规格电池。'},
              {title:'充电口维修',titleEn:'Charging Port',desc:'Type-C口松动、接触不良、只能慢充。'},
              {title:'主板维修',titleEn:'Motherboard',desc:'不开机、重启、充电IC故障、无服务。芯片级维修。'},
              {title:'后盖更换',titleEn:'Back Glass',desc:'Camera玻璃后盖碎裂更换。'},
