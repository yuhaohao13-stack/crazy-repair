    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-blue-700 via-blue-600 to-blue-500 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4"><a href="/computer-repair" className="inline-flex items-center gap-1.5 text-white/60 hover:text-white text-sm font-medium transition-colors"><ArrowLeft size={14} /> {t('电脑品牌', 'Computer Brands')}</a></div>
          ...
          <h1 className="text-3xl sm:text-5xl font-bold mb-3">{t('Lenovo 维修', 'Lenovo Repair')}</h1>
          <p className="text-green-100 text-lg mb-4">{t('Lenovo 全系列专业维修 | 威海', 'Lenovo All Series | Weihai')}</p>
          <p className="text-green-100 max-w-2xl">{t('Lenovo Find、Reno、A系列——屏幕碎了、电池不耐用了、充电口坏了，拿来给我看看。2007年至今奋斗在维修一线。', 'Lenovo Find, Reno, A series — cracked screen, battery drain, charging port issues. On the job since 2007.')}</p>
          <div className="flex gap-3 mt-6">
            ...
            <a href="https://wa.me/6596146709?text=Lenovo手机需要维修" target="_blank" className="bg-green-500 text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-green-600 shadow-lg">{t('💬 WhatsApp', '💬 WhatsApp')}</a>
          </div>
        </div>
      </section>
      <section className="py-14">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[{title:'屏幕更换',titleEn:'Screen',desc:'Lenovo Find/Reno/A系列屏幕更换，OLED/LCD。碎裂漏液触摸不灵。'},
              {title:'电池更换',titleEn:'Battery',desc:'Lenovo电池不耐用、鼓包、充不进电。原装规格电池。'},
              {title:'充电口维修',titleEn:'Charging Port',desc:'Type-C口松动、接触不良、只能慢充。'},
              {title:'主板维修',titleEn:'Motherboard',desc:'不开机、重启、充电IC故障、无服务。芯片级维修。'},
              {title:'后盖更换',titleEn:'Back Glass',desc:'Lenovo玻璃后盖碎裂更换。'},
