...
export default function XiaomiRepair() {
  ...
  const services = [
    { id: 'screen-replacement', icon:'🔧', title: t('屏幕更换', 'Screen Replacement'), desc: t('小米/Redmi 全系列屏幕，OLED/LCD都有。碎裂、漏液、触摸不灵。', 'Xiaomi/Redmi all series, OLED & LCD. Cracked, leaking, unresponsive touch.') },
    { id: 'battery-replacement', icon:'🔋', title: t('电池更换', 'Battery Service'), desc: t('小米电池不耐用、鼓包、充不进电。原装规格电池，性价比高。', 'Xiaomi battery issues: drain, swelling, no charge. OEM spec, great value.') },
    { id: 'motherboard-repair', icon:'🔬', title: t('主板维修', 'Motherboard'), desc: t('不开机、重启、充电IC、无服务。小米主板芯片级维修，性价比之选。', 'No power, boot loop, IC fault. Cost-effective component-level repair.') },
    { id: 'charging-port', icon:'🔌', title: t('充电口维修', 'Charging Port'), desc: t('Type-C口松动、接触不良、只能慢充。尾插小板更换。', 'Type-C loose, bad contact, slow charge only. Flex replacement.') },
    { id: 'back-glass', icon:'🔄', title: t('后盖/边框', 'Back Glass/Frame'), desc: t('小米玻璃后盖碎裂、中框变形。换后盖恢复防水胶。', 'Broken glass back, bent frame. Waterproof sealant restored.') },
    { id: 'flash-unlock', icon:'⚡', title: t('刷机/解锁', 'Flash/Unlock'), desc: t('MIUI系统卡顿、卡LOGO、忘记账号、解BL锁、刷国际版。', 'MIUI lag, boot loop, account lock, BL unlock, flash global ROM.') },
  ]
      ...
      <section className="py-16 sm:py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((s, i) => (
              <a key={i} href={'/xiaomi-repair/' + s.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-all group block">
                <div className="relative w-full aspect-[16/9] bg-gray-100 overflow-hidden">
                  <img src={'/images/services/xiaomi-repair-' + s.id + '.svg'} alt={s.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-xl bg-orange-50 flex items-center justify-center text-orange-600 text-lg shrink-0">{s.icon}</div>
                    <h3 className="font-bold text-gray-900 text-lg">{s.title}</h3>
                  </div>
                  <p className="text-sm text-gray-500 leading-relaxed">{s.desc}</p>
                  <div className="mt-3 text-orange-600 text-xs font-medium flex items-center gap-1">{t('查看详情 →', 'View Details →')}</div>
                </div>
              </a>
            ))}
          </div>
