  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="py-16 sm:py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((s, i) => (
              <a key={i} href={'/vivo-repair/' + s.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-all group block">
                {/* 配图 */}
                <div className="relative w-full aspect-[16/9] bg-gray-100 overflow-hidden">
                  <img
                    src={'/images/services/vivo-repair-' + s.id + '.svg'}
                    alt={s.title}
                ...
                <div className="p-5">
                  <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-xl bg-cyan-50 flex items-center justify-center text-cyan-600 text-lg shrink-0">
                      {[s.icon, '🔧'][0]}
                    </div>
                    <h3 className="font-bold text-gray-900 text-lg">{s.title}</h3>
                  </div>
                  <p className="text-sm text-gray-500 leading-relaxed">{s.desc}</p>
                  <div className="mt-3 text-cyan-600 text-xs font-medium flex items-center gap-1">{t('查看详情 →', 'View Details →')}</div>
                </div>
              </a>
            ))}
          </div>
      ...
      <section className="py-16 bg-gradient-to-br from-cyan-700 via-cyan-600 to-cyan-500 text-white text-center">
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <h2 className="text-2xl sm:text-3xl font-bold mb-4">{t('VIVO 出问题了？找我', 'VIVO issues? Hit me up')}</h2>
          <p className="text-cyan-200 mb-8">{t('免费检测，先报价后维修。添加微信发张照片就能初步判断。', 'Free diagnosis. Add WeChat, send a photo for a quick assessment.')}</p>
