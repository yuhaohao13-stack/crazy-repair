    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-gradient-to-br from-gray-700 via-gray-600 to-gray-500 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4"><a href="/phone-repair" className="inline-flex items-center gap-1.5 text-white/60 hover:text-white text-sm font-medium transition-colors"><ArrowLeft size={15} /> {t('手机品牌', 'Phone Brands')}</a></div>
          ...
          <h1 className="text-3xl sm:text-5xl font-bold mb-3">{t('Google 维修', 'Google Repair')}</h1>
          <p className="text-white/80 text-lg mb-4">{t('Google 全系列专业维修 | 威海', 'Google All Series | Weihai')}</p>
          <p className="text-white/80 max-w-2xl">{t('Google手机——屏幕碎了、电池不耐用了、充电口坏了，拿来给我看看。2007年至今奋斗在维修一线。', 'Google phone repair — screen, battery, port. On the job since 2007.')}</p>
          <div className="flex gap-3 mt-6">
            ...
            <a href="https://wa.me/6596146709?text=Google手机需要维修" target="_blank" className="bg-green-500 text-white font-semibold px-5 py-2.5 rounded-xl hover:bg-green-600 shadow-lg">{t('💬 WhatsApp', '💬 WhatsApp')}</a>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[{id:'screen-replacement', title:'屏幕更换',titleEn:'Screen',desc:'屏幕碎裂、漏液、触摸不灵。OLED/LCD都有。'},
              ...
              <a key={i} href={`/google-repair/${s.id}`} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md hover:-translate-y-0.5 transition-all block">
