    ...
    >
          ...
          {review.replies.map(reply => (
            <div key={reply.id} className={`p-2 rounded-lg ${reply.is_admin_reply ? 'bg-blue-50 border border-blue-100' : 'bg-gray-50'}`}>
              <div className="flex items-center gap-1 mb-0.5">
                ...
                {reply.is_pinned && <span className="text-xs bg-amber-100 text-amber-700 px-1 rounded">置顶</span>}
                {reply.is_admin_reply && !reply.is_pinned && <span className="text-xs bg-blue-100 text-blue-600 px-1 rounded">官方</span>}
              </div>
      ...
      <div className="flex items-center justify-between text-xs text-gray-400 border-t border-gray-50 pt-3 mt-auto">
        ...
        <div className="flex items-center gap-2">
          {review.replyCount > 0 && <span>{review.replyCount} 条回复</span>}
          ...
          {rest.map(r => (
            ...
            >
              ...
              <div className="flex items-center gap-2 shrink-0 text-xs text-gray-400">
                {r.replyCount > 0 && <span>{r.replyCount}条回复</span>}
                <span>{r.name}</span>
              </div>
            </div>
          ))}
...
function ReviewForm({ onClose, onSubmitted, user, router }) {
  ...
  const handleImageSelect = (e) => {
    ...
    if (images.length + files.length > 3) { setError('最多上传3张图片'); return }
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {images.length < 3 && (
            <button type="button" onClick={() => fileInputRef.current?.click()}
              ...
              <ImagePlus size={24} /><span className="text-xs mt-1">添加</span>
            </button>
          )}
        </div>
